...
@section('title', 'Edit Surat Masuk - SIMAS')

@section('content')
    <div class="d-flex justify-content-between align-items-center pt-3 pb-2 mb-4 border-bottom">
        <h1 class="h3 fw-bold text-dark">Edit Surat Masuk</h1>
    ...
    <div class="card shadow-sm border-0">
        ...
        <div class="card-body p-4 bg-white">
            <form action="{{ route('incoming-letters.update', $letter->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="sender" name="sender"
                            value="{{ old('sender', $letter->sender) }}" required>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="date" class="form-control" id="receipt_date" name="receipt_date"
                            value="{{ old('receipt_date', $letter->receipt_date) }}" required>
                    </div>
                    <div class="col-md-4">
                        <label for="status" class="form-label fw-bold text-secondary">Status Dokumen <span
                                class="text-danger">*</span></label>
                        <select class="form-select" id="status" name="status" required>
                            <option value="pending" {{ old('status', $letter->status) == 'pending' ? 'selected' : '' }}>
                                Pending / Belum Diproses</option>
                            <option value="dispositioned"
                                {{ old('status', $letter->status) == 'dispositioned' ? 'selected' : '' }}>Telah
                                Didisposisikan</option>
                            <option value="archived" {{ old('status', $letter->status) == 'archived' ? 'selected' : '' }}>
                                Selesai / Diarsipkan</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="mb-5">
                    <label for="file" class="form-label fw-bold text-secondary">Ganti File Scan Surat (Biarkan kosong
                        jika tidak ingin mengubah berkas)</label>
                ...
                <div class="d-flex justify-content-end border-top pt-4">
                    ...
                    <button type="submit" class="btn btn-primary-custom shadow-sm"><i
                            class="fa-solid fa-cloud-arrow-up me-1"></i> Perbarui Surat Masuk</button>
                </div>
            </form>
        </div>
    </div>
